...
export const Account = () => {
	...
	useEffect(() => {
		if (!loadingUser && user) {
			...
			form.setInitialValues({ email: user.email })
		}
	}, [user])

	return (
		<AccountLayout>
			{loadingUser && <Skeleton h='130px' mb={20} style={{ borderRadius: '10px' }} w='100%' />}
			{!loadingUser && (
				<form onSubmit={form.onSubmit((values) => onClickForm(values))}>
					...
					<Flex justify='flex-end' w='100%'>
						<Button bg='rgb(64, 192, 87)' disabled={loadingUpdate || !form.isDirty('email')} radius='lg' type='submit'>
							Сохранить
						</Button>
					</Flex>
				</form>
